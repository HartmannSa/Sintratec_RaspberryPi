...
class GUI(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # ********************************************************************************************************************************************#
        # Label Frames (farblich hinterlegte Unterteilung der GUI)
        # ********************************************************************************************************************************************#
            # General and Homing
        ...
        self.lbl_warning_z.grid(row=3,column=1,padx=(10,0),pady=(10,0), columnspan=2)
            # Scale (Schieberegler) Sledge
        self.scale_sledge = tk.Scale(self.lbl_frame_movements,orient='horizontal', resolution=0.01, bg=cfg.moveBeds_color,from_=cfg.SLEDGE_END_POS,to=1,length=220)
        ...
        tk.Label(self.lbl_frame_movements,text='workpiece bed\n(y) [mm]',bg=cfg.lblFrame_Movements_color).grid(row=4,column=1,padx=(10,0), columnspan=2)
            # Scale (Schieberegler) Workpiece bed
        self.scale_bed2 = tk.Scale(self.lbl_frame_movements, resolution = 0.01, orient='vertical', bg=cfg.moveBeds_color,from_=cfg.BED_MAX_POS,to=0.0)
        ...
        self.scale_bed2.grid(row=4,column=3,padx=(10,0),pady=(0,0),rowspan=2, columnspan=2)
            # Scale (Schieberegler) Powder Bed
        self.scale_bed1 = tk.Scale(self.lbl_frame_movements, orient='vertical', resolution=0.01, bg=cfg.moveBeds_color,from_=cfg.BED_MAX_POS,to=0)
        ...
        # 5. Zeile
            # Label Homing Warning (Workpiece Bed)
        ...
        self.lbl_warning_y.grid(row=5,column=1,padx=(10,0),pady=(10,0), columnspan=2)
            # Label Homing Warning (Powder Bed)
        ...
        self.lbl_bed_y_step.grid(row=6,column=1,padx=(10,0),pady=(10,0), columnspan=2, sticky='E')
            # Button ^,v (Steps) Workpiece Bed (y)
        ...
        # 7.Zeile
            # Entry(Eingabe) steps (for Workpiece Bed)
        ...
        self.bed_y_step_entry.bind('<Return>', self.btn_set_bed_y_step )
            # Button Apply (for Steps Workpiece Bed)
        tk.Button(self.lbl_frame_movements,text='Apply',command=self.btn_set_bed_y_step,width=cfg.BTN_WIDTH_SMALL).grid(row=7,column=3,padx=(10,0),pady=(0,0)) 
            # Button Apply (for Steps Powder Bed)
        tk.Button(self.lbl_frame_movements,text='Apply',command=self.btn_set_bed_x_step,width=cfg.BTN_WIDTH_SMALL).grid(row=7,column=6,padx=(0,0),pady=(0,0)) 
            # Entry(Eingabe) steps (for Powder Bed)
        ...
        self.lbl_mode_secure.grid(row=0,column=3,padx=(10,0),pady=(10,0))
            # Button "Turn on/off" (mode secure)
        ...
        self.lbl_layer_thickness_smoothing.grid(row=5,column=3,padx=(10,0),pady=(10,0))
            # Entry(Eingabefeld) for Layer thickness
...
# **********************************************************************************************
    def scale_sledge_fnc(self,var):
        # Färbe Scale und Button Move Rot, wenn Scale verschoben wurde
    ...
    def btn_set_sledge_step(self, event=None):
        ...
        max_val = float(cfg.SLEDGE_END_POS)-float(self.prntr.sledge_position)
...
# **********************************************************************************************
    def showInfoAfterHomed(self):
        messagebox.showinfo('All axes are homed!', 'Move the beds to certain positions to fill in the powder. Afterwards click ,Smooth powder´.')
...
# **********************************************************************************************      
    ...
    def process_dataline(self, line):
        ...
        # pos x | pos y | pos z | step x | step y | step z | speed x | speed y | speed z | Mode Secure
        try:
            data = line.split('|')
    ...
    def update_labels(self):
        # Red Marking in NOT SECURE MODE
        if (str(self.prntr.mode_secure)==str(1)):
            self.strvar_mode_secure.set('on')
